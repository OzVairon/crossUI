...
/**
 * Just a copy of {@link com.badlogic.gdx.scenes.scene2d.ui.TextArea}.
 * Has few fixes to work in y-down coordinate system.
 */
public class TextArea extends TextField {
	...
	protected void drawSelection (Drawable selection, Batch batch, BitmapFont font, float x, float y) {
		y = getY() + font.getLineHeight() - font.getAscent() / 2;
	...
	protected void calculateOffsets () {
		...
		if (!this.text.equals(lastText)) {
			...
			float maxWidthLine = this.getWidth()
				- (getStyle().background != null ? getStyle().background.getLeftWidth() + getStyle().background.getRightWidth() : 0);
			linesBreak.clear();
			int lineStart = 0;
			int lastSpace = 0;
			char lastCharacter;
	...
	public float getCursorX () {
		return textOffset + fontOffset + getStyle().font.getData().cursorX;
	}
...
//			System.out.println("x: " + x + ", y: " + y);
			...
			else if (y > getHeight() && cursorLine > firstLineShowing + linesShowing - 1) {
				cursorLine++;
				firstLineShowing++;
				showCursor();
			}
